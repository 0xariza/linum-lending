...
import { YieldToken } from "../yieldToken/YieldToken.sol";

/**
 * @title ILendingVaults
 * @dev Interface for the LendingVaults
 * @notice This interface defines all external functions, events, and structs for the LendingVaults
 */
interface ILendingVaults {
    ...
    struct CreditProfile {
        ...
        uint256 liquidatedPrincipal; // Principal repaid by liquidation - slot 5
        bool isActive; // Profile activation status - slot 6
        bool isBlacklisted; // User blacklist status - slot 6
    }
    ...
    struct BorrowPosition {
        address borrowedAsset; // Asset borrowed - slot 0
        ...
        uint256 borrowTime; // Timestamp of borrow - slot 6
        uint256 dueDate; // Repayment due date - slot 7
        bool isActive; // Position activation status - slot 8
    }
     ...
     * @param asset Address of the redeemed asset
     * @param amount Amount redeemed
     * @param shares Shares burned
     */
    event Redeem(address indexed user, address indexed asset, uint256 amount, uint256 shares);
    ...
    error ETHAmountShouldBeZero();
    error AmountTooSmall();
    error ETHNotAllowedForERC20();
    error SharesTooSmall();
    error InsufficientLiquidity();
    error BorrowAmountTooSmall();
    error BorrowLimitExceeded();
    error InsufficientCollateral();
    error CrossAssetCollateralNotSupported();
    error ETHAmountMismatch();
    error InvalidPositionIndex();
    error PositionNotActive();
    error RepayAmountTooSmall();
    error InsufficientETHSent();
    error PositionNotLiquidatable();
    error AssetAlreadySupported();
    error BorrowLimitTooSmall();
    error NameEmpty();
    error SymbolEmpty();
    error InvalidAssetAddress();
    error InvalidERC20Token();
    error TokenMissingDecimals();
    error AssetNotSupported();
    error CannotRemoveETH();
    error CannotRemoveInitialAssets();
    error AssetHasActiveReserves();
    error AssetHasActiveBorrows();
    error LimitTooSmall();
    ...
    /**
     * @dev Get the base borrow interest rate based on utilization rate
     * @param asset Address of the asset
    ...
    /**
     * @dev Get the user-specific borrow interest rate combining utilization rate and credit score
     * @param user Address of the user
     * @param asset Address of the asset being borrowed
     * @return User-specific interest rate in basis points
     */
    function getUserBorrowInterestRate(address user, address asset) external view returns (uint256);
    ...
    /**
     * @dev Get user's credit tier for easy categorization
     * @param user Address of the user
     * @return tier String representation of credit tier
     * @return collateralRatio Required collateral ratio
     * @return rateDiscount Interest rate discount in basis points
     */
    ...
    /**
     * @dev Accumulated protocol fees for each asset
     */
    function accumulatedFees(address asset) external view returns (uint256);

    /**
     * @dev YieldToken mapping for each asset
     */
    function saTokens(address asset) external view returns (YieldToken);
